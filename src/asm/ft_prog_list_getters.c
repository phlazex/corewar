...
static int	ft_get_code_line(t_project *project)
{
	...
	if (!(project->current_list->code_line =
			ft_strnew(project->current_list->command_size + 1)))
		return (0);
	i = 0;
	project->current_list->code_line[i++] =
			g_op_tab[project->current_list->command_num].id;
	if (g_op_tab[project->current_list->command_num].arg_type)
		project->current_list->code_line[i++] =
				(project->current_list->args_code[0] << 6) +
				(project->current_list->args_code[1] << 4) +
				(project->current_list->args_code[2] << 2);
	ft_get_args(project, &i);
	project->current_list->code_line[i] = 0;
	return (1);
}

static int	ft_get_program_line_while(t_project *project, size_t i)
{
	while (project->current_list)
	{
		if (!ft_get_code_line(project))
			return (0);
		ft_memcpy(project->program + i, project->current_list->code_line,
...
int			ft_get_program_line(t_project *project)
{
	...
	return (ft_get_program_line_while(project, i));
}
